...
BEGIN
  ...
  IF NOT (
    ...
    (OLD.status='APPROVED' AND NEW.status IN ('SENT','COMPLETED','CANCELLED','CLOSED')) OR
    (OLD.status='SENT' AND NEW.status IN ('PARTIALLY_RECEIVED','RECEIVED','COMPLETED','CANCELLED','CLOSED')) OR
    (OLD.status='PARTIALLY_RECEIVED' AND NEW.status IN ('RECEIVED','COMPLETED','CLOSED')) OR
    (OLD.status='RECEIVED' AND NEW.status IN ('COMPLETED','CLOSED'))
