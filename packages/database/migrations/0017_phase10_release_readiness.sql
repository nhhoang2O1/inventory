...
CREATE TABLE IF NOT EXISTS iam.auth_login_attempt (
...
CREATE INDEX IF NOT EXISTS ix_auth_attempt_throttle
...
CREATE INDEX IF NOT EXISTS ix_auth_session_active
  ON iam.auth_session (token_hash,expires_at)
  WHERE revoked_at IS NULL;

CREATE TABLE IF NOT EXISTS platform.release_gate_run (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS ix_release_gate_version
  ON platform.release_gate_run (release_version,environment,gate_type,executed_at DESC);

CREATE OR REPLACE VIEW platform.release_readiness_snapshot AS
SELECT
  now() AS observed_at,
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_auth_login_attempt_append_only') THEN
    CREATE TRIGGER trg_auth_login_attempt_append_only
    BEFORE UPDATE OR DELETE ON iam.auth_login_attempt
    FOR EACH ROW EXECUTE FUNCTION audit.reject_mutation();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_release_gate_run_append_only') THEN
