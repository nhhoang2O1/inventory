...
- Build PASS; 50 tests PASS after Core Gate hardening.
- Migrations 0001–0009 applied and repeat-safe.
...
- Reversal API creates a new append-only `REVERSAL` movement referencing the original; a unique guard prevents reversing the same movement twice.
- Query API covers On-hand balances, reservation lifecycle, ATP, IN_TRANSIT and ledger-balance reconciliation.
- Every source/destination warehouse in a multi-line posting is permission/scope checked.
- Fulfillment is bound to the reservation SKU/warehouse and occurs before the issue debit in the same transaction.
- A database constraint trigger rejects posting/reversal that would leave ATP negative; runtime rollback gate passed.
- Worker calls the idempotent reservation-expiry function in its polling transaction.
